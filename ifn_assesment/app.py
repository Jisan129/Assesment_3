...
ADMIN_SECRET_KEY = 'super_secret_admin_key_2025'
...
@app.route('/customer/book/<int:photographer_id>', methods=['GET', 'POST'])
@customer_required
def customer_book(photographer_id):
    """Customer can book a photographer"""
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute('''
                       INSERT INTO bookings (customer_id, photographer_id, booking_date, total_price)
                       VALUES (%s, %s, %s, %s)
                       ''', (session['id'], photographer_id, booking_date, total_price))
        mysql.connection.commit()
        cursor.close()
